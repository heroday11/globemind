...
LOG_DIR="$ROOT/logs"
PID_FILE="$LOG_DIR/daily_news_ingest_loop.pid"
LOG_FILE="$LOG_DIR/daily_news_ingest_loop.log"
LOOP_SCRIPT="$ROOT/deploy/daily_news_ingest_loop.sh"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

MANAGED_LOOP_SERVICE_ID="daily_news_ingest"
MANAGED_LOOP_LABEL="daily news ingest"
