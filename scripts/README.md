...
状态：current script navigation and safety boundary
适用范围：`scripts/` 下的 CI、运行时、数据作业和研究评估入口
...
[`manifest.json`](manifest.json) 是不改路径的导航和副作用分类索引：CI、运行时控制、
数据摄取/写入、研究评估和共享库各自声明了用途。它不授权执行；新增脚本必须先归类，
并在涉及文件、数据库、网络或进程时补充输入、checkpoint、回滚与 owner 说明。既有入口
保持兼容，不通过批量重命名来“整理”目录。
