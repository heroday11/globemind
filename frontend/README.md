...
前端 workspace 包含 Vue 主站（`vue_project/`）、React 金融终端（`financial-terminal/`）和共享包（`shared/`）。`knowledge_graph_backup/` 仅作为归档占位保留，不属于 workspace、样式扫描或生产构建。前端负责浏览器界面、路由、展示状态和 API 调用，不负责数据库写入、新闻抓取、模型推理或生产发布。
...
需要 Node.js（Vue 项目要求 `^20.19.0 || >=22.12.0`）及根目录 `package-lock.json`。在仓库根目录执行 `npm ci`，一次安装并校验全部 workspace；API/vLLM 地址通过各子项目 `.env.example` 配置。不要提交 `.env*` 中的密钥或把内部服务凭据打入浏览器包。
...
Vue 主站和金融终端为两个可独立构建的应用，主站的 `build` 会把金融终端构建到隔离输出的 `dist/fin-terminal/`；共享契约通过 `@globemind/shared` workspace 包提供。兼容知识图谱单元不应被视为完整产品模块。
